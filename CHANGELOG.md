...
## 5.5.45 — Shell-side suppression of cloud-drive source entries (2026-06-13)

继续收口壳子层过滤策略：除了家庭过滤开关外，来源返回的夸克、百度网盘、阿里云盘、UC 等云盘相关分类、卡片和筛选项也会被统一屏蔽，避免这类入口继续在配置中心或首页里露出。

### 新增 / 优化

- **来源项强制屏蔽**: 对夸克、百度网盘、阿里云盘、UC、迅雷、115、123pan、天翼、移动云盘等关键词做壳子层统一隐藏
- **原生内容流覆盖**: 首页、分类、详情、搜索等原生内容流统一应用屏蔽规则
- **WebHome 协作延续**: `fm.config()` 暴露的过滤关键词包含内置强制屏蔽项，页面侧可同步协作隐藏
