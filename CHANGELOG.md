...
## 5.5.46 — Respect source-side cloud-drive entries (2026-06-13)

调整家庭过滤为**可选壳子能力**，不再对来源返回的云盘相关展示项做强制统一屏蔽。

### 调整

- **保留家庭过滤**: 继续支持按标签/关键词过滤原生内容流
- **取消来源云盘强制屏蔽**: 不再替来源内容做统一云盘项隐藏，避免越界干预用户自带来源
- **WebHome 协作保持不变**: `fm.config()` 仍暴露家庭过滤状态和用户自定义关键词
